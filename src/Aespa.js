...
import "./Card.css";
import AespaItems from "./AespaItems";
import Navbar from "./Navbar";

const Aespa = ({ cart, handleClick }) => {
  const [data, setData] = useState(AespaItems);

  const filterRes = (categItem) => {
    const result = AespaItems.filter((curDate) => {
      ...
      <div className="card-categ-container">
        ...
        <div className="card-row">
          <div className="card-col">
            <h1 id="card-artist-title">Aespa</h1>
          </div>
        </div>

        <div className="card-row">
          <div className="card-col">
            <button className="card-btn" onClick={() => setData(AespaItems)}>
              All
            </button>
